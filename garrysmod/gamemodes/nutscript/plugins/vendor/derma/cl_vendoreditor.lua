local PANEL = {}
	function PANEL:Init()
		...
		self.anim = self:Add("DTextEntry")
		self.anim:Dock(TOP)
		self.anim:DockMargin(0, 4, 0, 0)
		self.anim:SetText(entity:getNetVar("anim", ""))
		self.anim.OnEnter = function(this)
			if (entity:getNetVar("anim"):lower() != this:GetText():lower()) then
				self:updateVendor("anim", this:GetText():lower())
			end
		end

		self.sound = self:Add("DTextEntry")
		self.sound:Dock(TOP)
		self.sound:DockMargin(0, 4, 0, 0)
		self.sound:SetText(entity:getNetVar("sound", ""))
		self.sound.OnEnter = function(this)
			if (entity:getNetVar("sound"):lower() != this:GetText():lower()) then
				self:updateVendor("sound", this:GetText():lower())
			end
		end
		...
		self.items.OnRowRightClick = function(this, index, line)
			if (IsValid(menu) and IsValid(left) and IsValid(left_bottom) and IsValid(right)) then
				menu:Remove()
				left:Remove()
				left_bottom:Remove()
				right:Remove()
		...
		for k, v in SortedPairs(nut.item.list) do
			...
			local current, max = entity:getStock(uniqueID)
	...
	function PANEL:OnRemove()
		if (IsValid(nut.gui.vendor) and IsValid(left) and IsValid(left_bottom) and IsValid(right)) then
